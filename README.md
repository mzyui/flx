...
> in the output without filtering. Both are `grab`/`find` flags.

The CLI is split into subcommands: `flx grab` scrapes the built-in
providers, `flx find` checks proxies against online judges, and a
`flx geo-update` subcommand pre-downloads the GeoLite2 database. Running
`flx` with no subcommand prints the help text.
...
Both flags are available on `flx grab` (and on `flx find` when its
